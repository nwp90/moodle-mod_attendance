...
/**
 * Renderer for the pasaf grade report
 *
...
/**
 * Custom renderer for the pasaf grade report
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradereport_pasaf_renderer extends plugin_renderer_base {
    ...
    public function view_user_selector($userid, $userview) {
        ...
        $options = array(GRADE_REPORT_PASAF_VIEW_USER => get_string('otheruser', 'gradereport_pasaf'),
                GRADE_REPORT_PASAF_VIEW_SELF => get_string('myself', 'gradereport_pasaf'));
        $select = new single_select($url, 'userview', $options, $userview, null);

        $select->label = get_string('viewas', 'gradereport_pasaf');
