 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradereport_pasaf_external extends external_api {
    ...
    protected static function check_report_access($courseid, $userid, $groupid = 0) {
        ...
        if (empty($userid)) {
            ...
            if (!groups_pasaf_groups_visible($course, $user->id)) {
    ...
    protected static function get_report_data($course, $context, $user, $userid, $groupid, $tabledata = true) {
        ...
        // Just one user.
        if ($user) {
            $report = new grade_report_pasaf($course->id, $gpr, $context, $userid);
            $report->fill_table();
            ...
            while ($userdata = $gui->next_user()) {
                ...
                $report = new grade_report_pasaf($course->id, $gpr, $context, $currentuser->id);
                $report->fill_table();
                            ...
                            'tabledata' => new external_multiple_structure(
                                new external_single_structure(
                                    array(
                                        ...
                                        'edit' => new external_single_structure(
                                            self::grades_table_column(), 'edit column', VALUE_OPTIONAL
                                        ),
    ...
    public static function view_grade_report($courseid, $userid = 0) {
        ...
        $report = new grade_report_pasaf($course->id, null, $context, $userid);
        $report->viewed();
