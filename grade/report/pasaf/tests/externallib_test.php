...
require_once($CFG->dirroot . '/grade/report/pasaf/externallib.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradereport_pasaf_externallib_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_grades_table_teacher() {
        ...
        $studentgrades = gradereport_pasaf_external::get_grades_table($course->id);
        $studentgrades = external_api::clean_returnvalue(gradereport_pasaf_external::get_grades_table_returns(), $studentgrades);
    ...
    public function test_get_grades_table_student() {
        ...
        $studentgrade = gradereport_pasaf_external::get_grades_table($course->id, $student1->id);
        $studentgrade = external_api::clean_returnvalue(gradereport_pasaf_external::get_grades_table_returns(), $studentgrade);
    ...
    public function test_view_grade_report() {
        ...
        $this->setUser($student1);
        $result = gradereport_pasaf_external::view_grade_report($course->id);
        $result = external_api::clean_returnvalue(gradereport_pasaf_external::view_grade_report_returns(), $result);
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check the event details are correct.
        $this->assertInstanceOf('\gradereport_pasaf\event\grade_report_viewed', $event);
        $this->assertEquals(context_course::instance($course->id), $event->get_context());
        $this->assertEquals($USER->id, $event->get_data()['relateduserid']);

        $this->setUser($teacher);
        $result = gradereport_pasaf_external::view_grade_report($course->id, $student1->id);
        $result = external_api::clean_returnvalue(gradereport_pasaf_external::view_grade_report_returns(), $result);
        $events = $sink->get_events();
        $event = reset($events);
        $sink->close();
        ...
        $this->setUser($student2);
        try {
            $studentgrade = gradereport_pasaf_external::view_grade_report($course->id, $student1->id);
    ...
    public function test_get_grade_items_teacher() {
        ...
        grade_set_setting($course->id, 'report_pasaf_showrank', 1);
        grade_set_setting($course->id, 'report_pasaf_showpercentage', 1);
        grade_set_setting($course->id, 'report_pasaf_showhiddenitems', 1);
        grade_set_setting($course->id, 'report_pasaf_showgrade', 1);
        grade_set_setting($course->id, 'report_pasaf_showfeedback', 1);
        grade_set_setting($course->id, 'report_pasaf_showweight', 1);
        grade_set_setting($course->id, 'report_pasaf_showcontributiontocoursetotal', 1);
        grade_set_setting($course->id, 'report_pasaf_showlettergrade', 1);
        grade_set_setting($course->id, 'report_pasaf_showaverage', 1);

        $studentgrades = gradereport_pasaf_external::get_grade_items($course->id);
        $studentgrades = external_api::clean_returnvalue(gradereport_pasaf_external::get_grade_items_returns(), $studentgrades);
        // No warnings returned.
        $this->assertCount(0, $studentgrades['warnings']);
    ...
    public function test_get_grade_items_student() {
        ...
        $studentgrades = gradereport_pasaf_external::get_grade_items($course->id, $student1->id);
        $studentgrades = external_api::clean_returnvalue(gradereport_pasaf_external::get_grade_items_returns(), $studentgrades);
        // No warnings returned.
        $this->assertCount(0, $studentgrades['warnings']);
