...
/**
 * Grade report showing only particular grade items.
 *
...
class transport implements apitransport {
    ...
    /**
     * Turn absolute URL returned from API into URL relative to API base URL
     * if possible.
     *
     * @param string $absoluteurl Absolute URL returned by API function
     * @return string
     */
    public function debased_url($absoluteurl) {
        $baseurl = $this->endpointurl;
        if (substr_compare($absoluteurl, $baseurl, 0, strlen($baseurl)) === 0) {
            $debasedurl = substr_replace($absoluteurl, '', 0, strlen($baseurl));
        }
        else {
            $debasedurl = $absoluteurl;
        }
        return $debasedurl;
    }
    ...
    public function get($url) {
        ...
        //error_log("getting from '$fullurl'...");
