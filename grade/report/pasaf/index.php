...
$PAGE->set_url(new moodle_url('/grade/report/pasaf/index.php', array('id'=>$courseid)));

if ($userview == 0) {
    $userview = get_user_preferences('gradereport_pasaf_view_user', GRADE_REPORT_PASAF_VIEW_USER);
} else {
    set_user_preference('gradereport_pasaf_view_user', $userview);
}
...
$gpr = new grade_plugin_return(array('type'=>'report', 'plugin'=>'pasaf', 'courseid'=>$courseid, 'userid'=>$userid));

/// last selected report session tracking
if (!isset($USER->grade_last_report)) {
    $USER->grade_last_report = array();
}
$USER->grade_last_report[$course->id] = 'pasaf'; //KJ - pasaf was user
...
if (has_capability('moodle/grade:viewall', $context)) { //Teachers will see all student reports
    ...
    $renderer = $PAGE->get_renderer('gradereport_pasaf');

    if ($userview == GRADE_REPORT_PASAF_VIEW_USER) {
        $viewasuser = true;
    } else {
        $viewasuser = false;
    }
    ...
    if (empty($userid)) {
        ...
        $gui->init();
        // Add tabs
        print_grade_page_head($courseid, 'report', 'pasaf');
        groups_print_course_menu($course, $gpr->get_return_url('index.php?id='.$courseid, array('userid'=>0)));

        if ($user_selector) {
            echo $renderer->graded_users_selector('pasaf', $course, $userid, $currentgroup, true);
        }
        ...
        while ($userdata = $gui->next_user()) {
            $user = $userdata->user;
            $report = new grade_report_pasaf($courseid, $gpr, $context, $asmapassessments, $user->id, $viewasuser);
            ...
            echo $OUTPUT->heading(get_string('pluginname', 'gradereport_pasaf') . ' - ' . $studentnamelink);
        ...
        $report = new grade_report_pasaf($courseid, $gpr, $context, $asmapassessments, $userid, $viewasuser);

        $studentnamelink = html_writer::link(new moodle_url('/user/view.php', array('id' => $report->user->id, 'course' => $courseid)), fullname($report->user));
        print_grade_page_head($courseid, 'report', 'pasaf', get_string('pluginname', 'gradereport_pasaf') . ' - ' . $studentnamelink,
        ...
        if ($user_selector) {
            ...
            echo $renderer->graded_users_selector('pasaf', $course, $userid, $currentgroup, $showallusersoptions);
        }
