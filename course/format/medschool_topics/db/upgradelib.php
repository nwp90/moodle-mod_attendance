...
/**
 * This method finds all courses in 'medschool_topics' format that have actual number of sections
 * bigger than their 'numsections' course format option.
 * For each such course we call {@link format_medschool_topics_upgrade_hide_extra_sections()} and
 * either delete or hide "orphaned" sections.
 */
function format_medschool_topics_upgrade_remove_numsections() {
    global $DB;
    ...
    $params = ['format1' => 'medschool_topics', 'format2' => 'medschool_topics', 'numsections' => 'numsections'];
    ...
    foreach ($needfixing as $courseid => $numsections) {
        format_medschool_topics_upgrade_hide_extra_sections($courseid, $numsections);
    }

    $DB->delete_records('course_format_options', ['format' => 'medschool_topics', 'sectionid' => 0, 'name' => 'numsections']);
}
 ...
 */
function format_medschool_topics_upgrade_hide_extra_sections($courseid, $numsections) {
    global $DB;
