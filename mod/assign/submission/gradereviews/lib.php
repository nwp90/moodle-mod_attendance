...
function assignsubmission_gradereviews_comment_display($gradereviews, $options) {
    ...
    foreach ($gradereviews as &$gradereview) {
        if ($gradereview->userid != $USER->id) {
            ...
            if (!has_capability('assignsubmission/gradereviews:caneditreviewgrade', context_user::instance($USER->id)) &&
                !has_capability('assignsubmission/gradereviews:caneditreviewgrade', context_course::instance($COURSE->id))) {
